...
class HTTPPluginRunner(Plugin):
    ...
    def start(self, args, evaluator, environment, ind, logger):
        ...
        with engine.Engine(port, args.get("strategy", ""), server_side=args["server_side"], environment_id=environment["id"], output_directory=output_path, enabled=args["server_side"], forwarder=forwarder) as eng:
    ...
    def get_args(command):
        ...
        parser.add_argument('--port', action='store', type=int, default=443, help='port to use')
